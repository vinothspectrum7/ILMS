...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, ScrollView, Dimensions, Modal, Platform, TouchableWithoutFeedback } from "react-native";
...
const Inv_CustomDropdown = ({
    ...
    const open = dropdownId === openDropdownId;
    const [query, setQuery] = useState("");
    const inputRef = useRef(null);
    const [dropdownPos, setDropdownPos] = useState({ x: 0, y: 0, w: 0, h: 0 });
        ...
        if (!disabled) {
            if (!open && inputRef.current) {
                inputRef.current.measureInWindow((x, y, w, h) => {
                    ...
                    onToggleOpen?.(dropdownId, true);
                });
            } else if (open) {
                onToggleOpen?.(dropdownId, false);
                setQuery("");
            }
        }
    };
    ...
    return (
        <View style={[styles.wrap, { zIndex: open ? 20 : 1 }, compact && styles.wrapCompact, disabled && styles.wrapDisabled, containerStyle]}>
            ...
            <TouchableOpacity
                ...
                ref={inputRef}
            >
                <Text style={[styles.valueText, !value && styles.placeholder, disabled && styles.valueDisabled]} numberOfLines={1} ellipsizeMode="tail">
                    {display}
                </Text>
                {open ? (
            ...
            >
                ...
                <View style={[
                    ...
                    <View style={styles.menu}>
                        ...
                        <ScrollView keyboardShouldPersistTaps="handled" style={{ maxHeight: menuMaxHeight }}>
                            {filtered.length === 0 ? (
...
const styles = StyleSheet.create({
    ...
    inputDisabled: { backgroundColor: "#aeaeaeff", borderColor: "#9D9FA3" },
