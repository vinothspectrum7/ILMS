...
import { Modal, View, Text, TouchableOpacity, Pressable, StyleSheet, Dimensions, ActivityIndicator } from 'react-native';
...
const ConfirmModalComponent = ({
  ...
  const modalWidth = useMemo(() => Math.max(280, Math.min(520, width * widthRatio)), [widthRatio]);
              ...
              <View style={styles.modalBody}>
                ...
                <View style={styles.buttonRow}>
                  <TouchableOpacity
                    ...
                    >
                    <ConfirmIcon width={50} height={50} fill={confirmColor} />
                    </TouchableOpacity>
                  <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={handleNo}
                    style={[{ backgroundColor: 'white' }]}
                    >
                    <CancelIcon width={50} height={50} fill={confirmColor} />
                    </TouchableOpacity>
                </View>
              </View>
            </>
...
const styles = StyleSheet.create({
  ...
  modalBox: { backgroundColor: 'white', borderRadius: 10, overflow: 'hidden' },
  modalHeader: { padding: 12, alignItems: 'center' },
  modalTitle: { fontSize: 18, fontWeight: 'bold', color: '#000' },
  modalBody: { padding: 20, alignItems: 'center' },
  modalMessage: { fontSize: 16, textAlign: 'center', marginBottom: 20, color: '#555' },
  buttonRow: { flexDirection: 'row', justifyContent: 'space-around', width: '60%' },
  circleButton: { width: 50, height: 50, borderWidth: 2, borderRadius: 25, justifyContent: 'center', alignItems: 'center' },
  statusBody: { paddingVertical: 28, paddingHorizontal: 24, alignItems: 'center' },
  statusText: { marginTop: 16, fontSize: 16, color: '#555', textAlign: 'center' },
});
