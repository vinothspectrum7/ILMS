...
import ConfirmSubInventoryIcon from '../../../assets/icons/org_success.svg';
...
export default function Org_TransferSummaryScreen() {
  ...
  const { OrgData, OrgnaizationTransferItems, editOrgnaizationTransferItems,resetOrgnaizationTransferItems,
    Orgtransferdetails
   } = useReceivingStore();
  ...
  const handleSaveLots = (lots, totalQty) => {
    ...
    const currentLine = OrgnaizationTransferItems[activeLineIndex];
    ...
    editOrgnaizationTransferItems(updated, activeLineIndex);
  ...
  const handleAddMore = () => {
    navigation.navigate('orgTransfer', { isAddMore: true });
  };

  const handleTransferPress = () => {
    if (!OrgnaizationTransferItems || OrgnaizationTransferItems.length === 0) return;
    setConfirmVisible(true);
  };

  const handleConfirmTransfer = () => {
    setConfirmVisible(false);
    console.log('SUB_INV_TRANSFER_SUBMIT', OrgnaizationTransferItems);
  ...
  useEffect(()=>{
    console.log(OrgnaizationTransferItems,"hasLineshasLineshasLineshasLines")
  },[hasLines])

  const hasLines = OrgnaizationTransferItems && OrgnaizationTransferItems.length > 0;
  const activeLine =
    activeLineIndex != null ? OrgnaizationTransferItems[activeLineIndex] : null;
  ...
  return (
    <View style={styles.root}>
          ...
          <View style={styles.row}>
            <View style={styles.fieldContainer}>
              ...
              <View style={{ alignItems: "flex-start" }}>
                <Text style={styles.orgvalue}>{OrgnaizationDetails?.orglabel} ORG</Text>
              </View>
            </View>
            <View style={styles.fieldContainer}>
            ...
            <View style={styles.fieldContainer}>
              <Text style={styles.orglabel}>Exp Receipt Date</Text>
      ...
      >
        {hasLines &&
          OrgnaizationTransferItems.map((line, index) => (
...
container: { flex: 1, backgroundColor: "#F7F8FB", paddingHorizontal: 16, paddingTop: 16 },
  ...
  itemCol: { flexDirection: "row",  flex:1 },
  cardTitleRow: {
  ...
  orgcard: { borderWidth: 1, borderColor: "#d0d0d0", borderRadius: 8, padding: 12, marginBottom: 16, backgroundColor: "#233E55", shadowColor: "#000", shadowOpacity: 0.05, shadowOffset: { width: 0, height: 2 }, shadowRadius: 4, elevation: 2 },
  row: { flexDirection: "row", justifyContent: "space-between" },
  fieldContainer: { flex: 1, marginHorizontal: 2 },
  orglabel: { fontFamily: "Mulish", fontWeight: "500", fontSize: 10, color: "#FFFFFF", marginBottom: 4 },
  orgvalue: { fontSize: 12, color: "#FFFFFF", fontWeight: "700" },
