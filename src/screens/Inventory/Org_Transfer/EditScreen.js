...
import { View, StyleSheet, Dimensions, TouchableOpacity, Modal, KeyboardAvoidingView, Platform, ScrollView, TextInput, Text } from 'react-native';
...
export default function Org_EditTransfer_Screen() {
  ...
  const OrgnaizationDetails = route?.params?.OrgnaizationDetails ?? null;
  ...
  const { editOrgnaizationTransferItems, OrgData } = useReceivingStore();
  ...
  const payloadForUpdate = useMemo(() => {
    ...
    const onUpdate = useCallback(() => {
        ...
        if (EditIndex !== null && EditIndex >= 0) {
          console.log(EditIndex,"EDITINDEXEIDT");
          console.log(payloadForUpdate,"payloadForUpdate");
      editOrgnaizationTransferItems(payloadForUpdate, EditIndex);
    }else{
    ...
    }, [itemToEdit,EditIndex, payloadForUpdate, editOrgnaizationTransferItems, navigation]);
