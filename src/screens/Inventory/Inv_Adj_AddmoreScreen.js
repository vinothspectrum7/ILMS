...
export default function Inv_Adj_AddmoreScreen() {
  const navigation = useNavigation();
  const route = useRoute();
  const initialType = route?.params?.adjustmentType || "Issue";
  ...
  const [receiptFields, setReceiptFields] = useState({ itemId: null, toSubId: null, toLocId: null, uomId: null, qty: 0, maxQty: 999999 });
  ...
  }, [issueFields.itemId, issueFields.fromSubId, receiptFields.itemId, receiptFields.toSubId, adjustType, OrgData?.selectedOrg]);
  ...
  const onAdd = useCallback(() => {
    ...
    Toast.show({ type: "success", text1: "Item added successfully", position: "top", visibilityTime: 1200 });
  ...
  return (
    <View style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <View style={styles.itemRow}>
            <Inv_CustomDropdown
              ...
              onChange={(id) => {
                if (adjustType === "Issue")
                  setIssueFields({ itemId: id, fromSubId: null, fromLocId: null, uomId: null, qty: 0, maxQty: 0 });
                else
                  setReceiptFields({ itemId: id, toSubId: null, toLocId: null, uomId: null, qty: 0, maxQty: 999999 });
              }}
              options={itemOptions}
              idKey="value"
              nameKey="label"
      ...
      <Inv_FooterBtnComponent
        leftLabel="View Summary"
        rightLabel="Add"
        rightEnabled={isAddEnabled}
        onLeftPress={() => navigation.navigate("Inv_Adj_SummaryScreen", { adjustmentType: adjustType })}
        onRightPress={onAdd}
      />
