...
import Rec_LotModalViewPopup from '../../components/receive/Rec_LotModalViewPopup';
...
const Rec_ViewReceivedItemDetailsScreen = () => {
  ...
  const [inspectRowsMap, setInspectRowsMap] = useState({});
  ...
  const canInspectTab = isInspectionRequired;
  const canPutAwayTab = isStandardReceipt || isInspectionRequired;
  ...
  const isInspectionFullyCompleted = useMemo(() => {
    if (!isInspectionRequired) return true;
  ...
  const isPutAwayFullyCompleted = useMemo(() => {
    if (!(isStandardReceipt || isInspectionRequired)) return true;
    if (!current) return false;
  ...
  const putAwaySavedSerialsCount = currentSavedSerials.length;
          ...
          <View style={styles.cardReceive}>
            <View style={styles.tabRow}>
              ...
              <TouchableOpacity
                style={styles.tabWrapper}
                activeOpacity={0.9}
                onPress={() => isInspectionRequired && setActiveTab('Inspect')}
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.9}
                onPress={() => {
                  if (isStandardReceipt || isInspectionRequired) {
                    setActiveTab('PutAway');
                  }
                }}
                disabled={!(isStandardReceipt || isInspectionRequired)}
              >
                ...
                <View style={styles.inspectRequiredWrap}>
                  ...
                  <View style={styles.putAwayTable}>
                    <View style={styles.putAwayTableHeader}>
                      ...
                      <Text style={[styles.putAwayTh, styles.putAwayThMid]}>Inspection Status</Text>
                      <Text style={[styles.putAwayTh, styles.putAwayThRight]}>Action</Text>
                    </View>

                    {currentPutAwayRows.map((r, idx) => {
                      const isCompleted = !!r.isCompleted;
                      const isDeliveryCompleted = r.deliveryStatus === 'Completed';
                      ...
                      return (
                        ...
                        >
                          ...
                          <View style={[styles.putAwayTd, styles.putAwayTdMid]}>
                            <View style={styles.inspectionStatusAccepted}>
                              <Text style={styles.inspectionStatusAcceptedText}>
                                Accepted
                              </Text>
                            </View>
                          </View>
                            ...
                            <TouchableOpacity
                              ...
                              onPress={() => {
                                setSelectedPutAwayRow({
                                  ...r,
                                  lotNumber: currentLotLines[idx]?.lotNumber || '',
                                  lotIndex: idx,
                                });
                                setPutAwayRowModalVisible(true);
                              }}
          ...
          {activeTab === 'Receive' && current && !isInspectionRequired && !isStandardReceipt && (
            <>
              ...
              <View style={styles.cardDetails}>
                ...
                {(itemType === 'Lot' || itemType === null || itemType === undefined) && (
