...
import ASNinfoCardComponent from '../components/IC_ASNinfoCardComponent';
import ASNListCardComponent from '../components/IC_Asnlistcardcomponent';
import AsnHeaderComponent from '../components/IC_AsnTableHeader';
...
import { GetSavedSingleASN, Save_Receive_Qty } from '../api/ApiServices';
...
const IC_AsnReceiptScreen = () => {
  ...
  const seededOnceRef = useRef(new Set());
  ...
  const iface = useMemo(() => activeASN?.interface_header_id ?? activeASN?.interface_id ?? null, [activeASN?.interface_header_id, activeASN?.interface_id]);
  const asnKey = useMemo(() => (activeASN?.asn_id ? `${activeASN.asn_id}:${iface || '-'}` : null), [activeASN?.asn_id, iface]);
  ...
  const seedFromCheckedReceiptsOnce = useCallback((poGroups) => {
    if (!asnKey || seededOnceRef.current.has(asnKey)) return;
    const pendingSelected = [];
    const editedByPO = new Map();
    for (const po of poGroups) {
      if (String(po?.po_status).toUpperCase() === 'FULLY RECEIVED') continue;
      const hasChecked = (po.line_items || []).some((li) => Array.isArray(li?.receipts) && li.receipts.some((r) => r?.is_checked === true));
      if (!hasChecked) continue;
      pendingSelected.push(String(po.po_id));
      const seeded = (po.line_items || []).map((li) => {
        const prefilled = Array.isArray(li?.receipts) ? li.receipts.reduce((acc, r) => acc + (r?.is_checked ? Number(r?.received_qty ?? 0) : 0), 0) : 0;
        return { ...deepClone(li), receiving_qty: clampASN(li, prefilled) };
      });
      editedByPO.set(String(po.po_id), seeded);
    }
    if (pendingSelected.length === 0) {
      seededOnceRef.current.add(asnKey);
      return;
    }
    const current = (asnSelectedPOIds || []).map(String);
    const mergedIds = Array.from(new Set([...current, ...pendingSelected]));
    setAsnSelectedPOIds(mergedIds);
    pendingSelected.forEach((poId) => {
      const lines = editedByPO.get(poId) || [];
      setAsnEditedLinesForPO(poId, lines);
    });
    const results = [];
    const selSet = new Set(mergedIds.map(String));
    const selectedPOs = poGroups.filter((it) => selSet.has(String(it.po_id)));
    for (const po of selectedPOs) {
      const merged = (editedByPO.get(String(po.po_id)) || po.line_items || []).map((li) => ({ ...li, receiving_qty: clampASN(li, Number(li?.receiving_qty ?? 0)) }));
    ...
    seededOnceRef.current.add(asnKey);
  }, [asnKey, asnSelectedPOIds, setAsnSelectedPOIds, setAsnEditedLinesForPO, initAsnSelectedLines]);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const run = async () => {
        if (!activeASN?.asn_id) {
          setPhase('error');
          Toast.show({ type: 'error', text1: 'Missing ASN ID', text2: 'Cannot fetch saved ASN without asn_id', position: 'top', visibilityTime: 3000 });
          return;
        }
        try {
          setPhase('loading');
  ...
  return {
    ...
    disabled,
          ...
          setPhase('success');
          seedFromCheckedReceiptsOnce(grouped);
        } catch (e) {
          if (cancelled) return;
          setItems([]);
          setPhase('error');
          const msg = e && typeof e.message === 'string' ? e.message : 'Failed to fetch saved ASN';
          Toast.show({ type: 'error', text1: 'Unable to load Saved ASN', text2: msg === 'timeout' ? 'Request timed out. Please retry.' : msg, position: 'top', visibilityTime: 3500 });
        }
      };
      run();
      return () => { cancelled = true; };
    }, [activeASN?.asn_id, iface, seedFromCheckedReceiptsOnce, setAsnHeader])
  );
  ...
  const handleReceive = () => {
    ...
    navigation.navigate('IC_podetailsummary', { fromScan: false, scannedAsnId, scannedAsnNumber });
  };
...
export default IC_AsnReceiptScreen;
